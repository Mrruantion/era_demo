...
W.getOpenId = function (needweixin, s) {
    ...
    W.setCookie("__login_redirect_uri__", location.href, -15);
    var u = encodeURIComponent(WiStorm.config.wx_login);
    var _url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + WiStorm.config.wx_app_id + "&redirect_uri=" + u + "&response_type=code&scope=snsapi_base&state=" + s + "#wechat_redirect";
    top.location = _url;
}
...
W._loginSuccess = function (res) {
    ...
    // 2017-03-31 每个公众号保存一份
    if (W.getSetting("account") == '15678156054') {
        W.alert('set wx app id = ' + WiStorm.config.wx_app_id);
    }
...
if (W.getSetting("account") == '15678156054') {
    alert('get wx app id = ' + WiStorm.config.wx_app_id);
}
if (WiStorm.config.wx_app_id) {
    window._user = W.getSetting("user" + WiStorm.config.wx_app_id);
} else {
